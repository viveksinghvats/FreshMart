...
class _OrderDetailsState extends State<OrderDetails> {
  bool isLoading = false, isRatingSubmitting = false, isCancelLoading = false;
  var orderHistory;
  String currency;
  double rating;
  ...
  cancelOrder(id) async {
    ...
    await LoginService.cancelOrder(cancelOrderData).then((onValue) {
      print('on cal $onValue');
      try {
        if (onValue['response_code'] == 200) {
          if (mounted) {
            showSnackbar(onValue['response_data']);
            setState(() {
              cancelOrderList = onValue['response_data'];
                            ...
                            child: Column(
                              children: <Widget>[
                                ...
                                Row(
                                  mainAxisAlignment: MainAxisAlignment.start,
                                  children: <Widget>[
                                    Text(MyLocalizations.of(context).location,
                                        style: textBarlowMediumBlack()),
                                    SizedBox(width: 5),
                                    Expanded(
                                      child: Text(
